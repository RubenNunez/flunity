...
void main() {
  test(
    'stub WebGL build serves with correct headers + shim is fetchable',
    () async {
      ...
      Future<HttpClientResponse> get(String path) async {
        final req = await client.getUrl(
          Uri.parse('http://${server.host}:${server.port}$path'),
        );
        return req.close();
      }
      ...
      expect(
        indexResp.headers.value('cross-origin-opener-policy'),
        'same-origin',
      );
      expect(
        indexResp.headers.value('cross-origin-embedder-policy'),
        'require-corp',
      );
