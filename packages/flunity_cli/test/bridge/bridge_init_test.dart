...
void main() {
  ...
  setUp(() {
    ...
    File(
      p.join(libUnity.path, 'unity_webgl_screen.dart'),
    ).writeAsStringSync('// screen');
    File(
      p.join(libUnity.path, 'unity_webgl_config.dart'),
  ...
  test(
    'adds dep, copies template files, leaves missing index.html alone',
    () async {
      ...
      expect(
        File(
          p.join(tmp.path, 'flutter_app/lib/unity/unity_webgl_screen.dart'),
        ).existsSync(),
        isTrue,
      );
      expect(
        File(
          p.join(tmp.path, 'unity_project/Assets/Scripts/FlunityBridge.cs'),
        ).existsSync(),
        isTrue,
      );
    },
