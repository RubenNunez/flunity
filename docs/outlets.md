...
**Status: works on iOS and Android.** Outlets are transport-agnostic — the same `flunity.invoke` / `flunity.find` API runs on both targets.
...
| No Unity view mounted | `FlunityNotAttachedException` | Mount a `FlunityNativeView` (or attach a custom transport) before calling outlets. On desktop (no Unity), outlets are unavailable. |
